...
* BREAKING CHANGES:
  * Feat: Completed migration to Federated Plugin structure. This requires one change:
  ...
  ```
  * Deprecated: [Android] Phone account registration methods (`registerPhoneAccount`, `hasRegisteredPhoneAccount`, `isPhoneAccountEnabled`, `openPhoneAccountSettings`) and permission methods (`hasManageOwnCallsPermission`, `requestManageOwnCallsPermission`) are no longer required for in-app calling. These methods are deprecated and will be removed in a future release. They now return graceful defaults.
* FEAT: [Android] Major Android in-app calling refactor: Replaced `ConnectionService` architecture with custom `TVCallManager` + foreground `Service` for audio session management. Calls now remain in-app instead of popping to system call UI. Minimum SDK remains 26.
* Feat: [Android] Upgrade Twilio Voice SDK to v7.x with modernized Kotlin code (scope functions, extension functions, null-safety improvements)
* Feat: [iOS] Modernize Swift code with optional binding, guard statements, and contemporary patterns. Upgrade Twilio Voice SDK to v6.13.0+.
